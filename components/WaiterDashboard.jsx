...
import { BellRing, ChefHat, CheckCircle2, Clock3, Loader2, RefreshCw, ArrowRightCircle } from 'lucide-react';
...
export default function WaiterDashboard() {
  ...
  useEffect(() => {
    ...
    const storedId = localStorage.getItem('waiterId') || localStorage.getItem('prathomix_waiter_staff_id') || sessionStorage.getItem('prathomix_waiter_staff_id');
  ...
  function handleLogout() {
    ...
    localStorage.removeItem('prathomix_waiter_staff_id');
    sessionStorage.removeItem('prathomix_waiter_staff_id');
  ...
  return (
    <div className="min-h-screen bg-[#f6efe3] text-stone-900 px-4 py-6 md:px-8">
      <div className="mx-auto max-w-7xl space-y-6">
        <div className="flex flex-col gap-4 rounded-3xl border border-stone-200 bg-white/90 p-5 shadow-sm md:flex-row md:items-start md:justify-between">
          <div>
            <div className="mb-2 inline-flex items-center gap-2 rounded-full border border-amber-200 bg-amber-50 px-3 py-1 text-[11px] font-semibold uppercase tracking-[0.25em] text-amber-700">
              <ChefHat className="h-3.5 w-3.5" /> Waiter Live Board
            </div>
            <h1 className="text-2xl font-bold">Waiter Dashboard</h1>
            <p className="mt-1 text-sm font-medium text-amber-700">Welcome, {waiterName}</p>
            <p className="mt-1 text-sm text-stone-600">Track incoming orders and stay synced with the kitchen in real time.</p>
          </div>
          ...
          <div className="flex flex-wrap items-center gap-3 md:justify-end">
            <button
              ...
              className="inline-flex items-center gap-2 rounded-xl border border-amber-200 bg-amber-50 px-4 py-2 text-sm font-medium text-amber-800 transition hover:bg-amber-100">
              Logout
            </button>
            <button
              type="button"
        ...
        {loading ? (
          <div className="flex items-center justify-center rounded-3xl border border-stone-200 bg-white py-16 text-stone-500">
            <Loader2 className="mr-2 h-5 w-5 animate-spin" /> Loading waiter board…
          </div>
